...
class KAISTPed(data.Dataset):
    ...
    def __init__(self, args, condition='train'):
        ...
        src_folder = f'../data/kaist-rgbt/annotations_paired_miss_small_{args.MP}/'
        base_dst_folder = f'../data/kaist-rgbt/annotations_paired_miss_small_{args.MP}_Edit'
        ...
        if condition == "train":
            if "exist" in self.image_set and not self.args.Make_GT:
                self._annopath = os.path.join('%s', f'annotations_paired_miss_{self.args.SA_type}_{self.args.MP}_Edit1', '%s', '%s', '%s', '%s.txt')  #Sparse + Dynamic
                #self._annopath = os.path.join('%s', f'annotations_paired', '%s', '%s', '%s', '%s.txt') #Fully Supervised
                #self._annopath = os.path.join('%s', f'annotations_paired_Edit', '%s', '%s', '%s', '%s.txt') #Fully Supervised + Dynamic
            elif "exist" in self.image_set and self.args.Make_GT:
                self._annopath = os.path.join(self.dst_folder, '%s', '%s', '%s', '%s.txt')
            else:
                self._annopath_exist = os.path.join('%s', f'annotations_paired_miss_{self.args.SA_type}_{self.args.MP}_Edit1', '%s', '%s', '%s', '%s.txt')
                #self._annopath_exist = os.path.join('%s', f'annotations_paired', '%s', '%s', '%s', '%s.txt')
                #self._annopath = os.path.join('%s', f'annotations_paired_Edit', '%s', '%s', '%s', '%s.txt')
                self.ids_exist = list()
    ...
    def __getitem__(self, index): 
        vis, lwir,vis2, lwir2, boxes, labels, boxes2, labels2, return_id,dst_folder = self.pull_item(index)
        return vis, lwir,vis2, lwir2, boxes, labels, boxes2, labels2, torch.ones(1,dtype=torch.int)*index, return_id, dst_folder
    ...
    def select_gt_sample_brightness(self, rgb_img, ir_img):
        ...
        with open(f"img_rgb_avg_{self.args.MP}_fusion.txt") as f:
            lines = f.readlines()
        ...
        rgb_directory = f'../data/KAIST_gt_samples_{self.args.MP}/visible'
        starting_with = min_bright[0].replace("/", "_")
        rgb_files = self.find_files(starting_with, rgb_directory)
        ir_directory = f'../data/KAIST_gt_samples_{self.args.MP}/lwir'
        starting_with = min_bright[0].replace("/", "_")
    ...
    def pull_item(self, index):
                ...
                for i in range(number_of_mix):
                    ...
                    if vis_gt_sample is not None and lwir_gt_sample is not None:
                        if len(vis_boxes) != 0 and len(lwir_boxes) != 0:
                            ...
                            #print(f"insetion_box : {insertion_box}")
        ...
        if self.co_transform_weak is not None:
            vis2, lwir2, boxes_vis2, boxes_lwir2, _ = self.co_transform_weak(vis, lwir, boxes_vis, boxes_lwir)
        ...
        return vis, lwir, vis2, lwir2, boxes, labels, boxes2, labels2, return_id, self.dst_folder
    ...
    def collate_fn(self, batch):
        ...
        for b in batch:
            ...
            labels2.append(b[7])
            index.append(b[8])
            retrun_id.append(b[9])
            dst_folder.append(b[10])
        ...
        return vis, lwir, vis2, lwir2, boxes, labels, boxes2, labels2, index, retrun_id, dst_folder
