...
from model_resnet import SSDResNet, SSDResNet_3Way
...
def init_model():
    ...
    if train_conf.train_mode == "baseline":
        if train_conf.backbone == "MLPD":
            model = SSD300(n_classes=args.n_classes)
        else:
            model = SSDResNet(n_classes=args.n_classes)
    else:
        if train_conf.backbone == "MLPD":
            model = SSD300_3Way(n_classes=args.n_classes)
            print(f"Backbone: {train_conf.backbone}")
        else:
            model = SSDResNet_3Way(n_classes=args.n_classes)
            print(f"Backbone: {train_conf.backbone}")
                ...
                for line in results[key]:
                    ...
                    if text == "PL":
                        ...
                        if float(score) >= 0.5:
                            temp_box[key-1].append([float(x), float(y), float(w), float(h)])
                            temp_score[key-1].append(float(score))
                if len(temp_score[key-1]) != len(temp_box[key-1]):
                    print(f"Not Fit!")
                    print(f"i : {i}, key: {key}")
                    print(f"temp_score: {temp_score[key-1]}, temp_box: {temp_box[key-1]}")
