...
def val_epoch(model: SSD300, dataloader: DataLoader, input_size: Tuple, inference_func, min_score: float = 0.1) -> Dict:
    model.eval()
    ...
    with torch.no_grad():
        for i, blob in enumerate(dataloader):
            image_vis, image_lwir, _, _,_, _,_, _, indices,_,_ = blob
